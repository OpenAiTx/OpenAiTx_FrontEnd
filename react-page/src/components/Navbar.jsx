...
const MobileLanguageSelector = ({ availableLanguages, currentLanguage, onLanguageChange, onClose, t }) => {
    ...
    return (
        <div className="px-0 py-0">
            ...
            {isExpanded && (
                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="mt-2 px-3 grid grid-cols-2 gap-2">
                    {availableLanguages.map((lang) => (
                        <motion.button
                            ...
                            className={`text-left px-3 py-2 text-sm rounded-md transition-colors flex items-center justify-between ${lang.code === currentLanguage ? "bg-primary/10 text-primary" : "text-foreground hover:bg-muted/50"}`}
                            whileHover={{ scale: 1.02 }}
                            whileTap={{ scale: 0.98 }}>
                            <span className="truncate">{lang.name}</span>
                            {lang.code === currentLanguage && <span className="text-primary">✓</span>}
                        </motion.button>
                    ))}
                </motion.div>
            )}
...
const Navbar = ({ darkMode, setDarkMode }) => {
    ...
    return (
        <motion.nav className="fixed top-0 left-0 right-0 z-50 bg-background/80 backdrop-blur-md border-b border-border" initial={{ y: -100 }} animate={{ y: 0 }} transition={{ duration: 0.5 }}>
            <div className="container mx-auto px-4">
                <div className="flex items-center justify-between h-16">
                        ...
                        <DropdownMenu modal={false}>
                            ...
                            <DropdownMenuContent align="end" className="w-36" sideOffset={8}>
                                {availableLanguages.map((language) => (
                                    <DropdownMenuItem key={language.code} onClick={() => handleLanguageChange(language.code)} className="flex items-center justify-between cursor-pointer">
                                        <span>{language.name}</span>
                                        {language.code === i18n.language && <span className="text-primary">✓</span>}
                                    </DropdownMenuItem>
                                ))}
                            </DropdownMenuContent>
                        </DropdownMenu>
                    ...
                    <div className="md:hidden">
                        <motion.button onClick={toggleMenu} className="p-2 rounded-md text-muted-foreground hover:text-foreground hover:bg-muted/50 transition-colors" whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} aria-label={t("nav.menu")}>
                ...
                {/* Mobile Navigation */}
                {isMenuOpen && (
                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="md:hidden border-t border-border z-50">
                        <div className="px-2 pt-2 pb-3 space-y-1 min-h-[calc(100vh-4rem)]">
                            ...
                            <MobileLanguageSelector availableLanguages={availableLanguages} currentLanguage={i18n.language} onLanguageChange={handleLanguageChange} onClose={closeMenu} t={t} />
                        </div>
                    </motion.div>
                )}
            </div>
